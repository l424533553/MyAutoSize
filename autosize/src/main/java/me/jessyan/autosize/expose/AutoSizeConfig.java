...
public final class AutoSizeConfig {
     ...
     * {@link AutoSizeConfig #isBaseOnWidth} 为 {@code true} 时代表以宽度等比例缩放, {@code false} 代表以高度等比例缩放
     * {@link AutoSizeConfig #isBaseOnWidth} 为全局配置, 默认为 {@code true}, 每个 {@link Activity} 也可以单独选择使用高或者宽做等比例缩放
     */
    private boolean isBaseOnWidth = true;
    /**
     * 此字段表示是否使用设备的实际尺寸做适配
     * {@link AutoSizeConfig #isUseDeviceSize} 为 {@code true} 表示屏幕高度 {@link #mScreenHeight} 包含状态栏的高度
     * {@link AutoSizeConfig #isUseDeviceSize} 为 {@code false} 表示 {@link #mScreenHeight} 会减去状态栏的高度, 默认为 {@code true}
     */
    ...
    /**
     * 设置屏幕适配监听器，监听屏幕横竖的切换,在横竖切换时，需要进行部分额外操作，
     * 确保屏幕能适配，具体方操作在{@link OnAdaptListener}中请详细阅读
     *
     ...
     * 是否让框架支持自定义 Fragment 的适配参数, 由于这个需求是比较少见的, 所以须要使用者手动开启
     *
     * @param customFragment {@code true} 为支持,一般没有这个需求就不要开启
     */
