 ...
 * ================================================
 * <p>
 * 管理 AndroidAutoSize 支持的所有单位, AndroidAutoSize 支持五种单位 (dp、sp、pt、in、mm),使用者随意配置自己想使用的单位类型
...
public class UnitsManager {
    ...
    /**
     * 设置设计图尺寸，一般专为保存副单位的尺寸设计，不会被AndroidManifest中设置的尺寸覆盖。
     * <p>
     *
     ...
     *
     * @param supportDP {@code true} 为支持，默认支持。 {@code false} 为不支持
     */
     ...
     * AndroidAutoSize 默认开启对 sp 的支持,如果关闭对 sp 的支持就在布局时使用副单位填写字体的尺寸。
     * sp开启支持对其他三方库控件影响不大,所以也可以开启 sp 的支持。
     *
    ...
    /**
     * AndroidAutoSize 默认不支持副单位, 调用 {@link UnitsManager #setSupportSubunits() }可选择一个自己心仪的副单位, 并开启对副单位的支持
     * 让 AndroidAutoSize 以什么单位为副单位, 在 pt、in、mm 这三个冷门单位中选择一个即可, 三个效果都是一样的，选择了哪个副单位就在
     * 布局页面中用选择的副单位进行布局
     * 默认为 {@link Subunits#NONE}, 即不支持副单位, 详情请看类文件的注释 {@link Subunits}
     *
